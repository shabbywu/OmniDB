...
def cleanup_thread():
    while True:
        ...
        for client in list(global_object):
            ...
            client_timeout_reached = datetime.now() > global_object[client][
                "last_update"
            ] + timedelta(0, 3600)
            ...
            for tab_id in list(global_object[client]["tab_list"]):
                try:
                    # Tab reached timeout
                    tab_timeout_reached = datetime.now() > global_object[client][
                        "tab_list"
                    ][tab_id]["last_update"] + timedelta(0, 3600)

                    if (
                        client_timeout_reached
                        or tab_timeout_reached
                        or global_object[client]["tab_list"][tab_id]["to_be_removed"]
                        == True
                    ):
...
def close_tab_handler(p_client_object, p_tab_object_id):
    try:
        ...
        if (
            ...
            or tab_object["type"] == "console"
            or tab_object["type"] == "connection"
            or tab_object["type"] == "edit"
        ):
            try:
            ...
            try:
                ...
                None
            try:
                tab_object["omnidatabase_control"].v_connection.Terminate(
                    tab_object["debug_pid"]
                )
            except Exception:
                None
            try:
...
def create_tab_object(p_session, p_tab_id, p_object):
    v_object = p_object
...
def get_client_object(p_client_id):
    ...
    try:
        ...
        # TODO: replace 'polling_lock', 'returning_data_lock' to logic lock but not system lock
        client_object = {
            "id": p_client_id,
            "polling_lock": threading.RLock(),
            "returning_data_lock": threading.RLock(),
            "returning_data": [],
            "tab_list": {},
...
def get_database_object(
    ...
    try:
        ...
        v_tab_object = create_tab_object(
            p_session, p_tab_id, {"omnidatabase": None, "type": "connection"}
        )
...
def get_database_tab_object(
    ...
    if p_tab_object["omnidatabase"] == None or (
        v_global_database_object.v_db_type != p_tab_object["omnidatabase"].v_db_type
        or v_global_database_object.v_connection.v_host
        != p_tab_object["omnidatabase"].v_connection.v_host
        or str(v_global_database_object.v_connection.v_port)
        != str(p_tab_object["omnidatabase"].v_connection.v_port)
        or v_current_tab_database != p_tab_object["omnidatabase"].v_active_service
        or v_global_database_object.v_active_user
        != p_tab_object["omnidatabase"].v_active_user
        or v_global_database_object.v_connection.v_password
        != p_tab_object["omnidatabase"].v_connection.v_password
    ):
    ...
    if p_attempt_to_open_connection and (
        not p_tab_object["omnidatabase"].v_connection.v_con
        or p_tab_object["omnidatabase"].v_connection.GetConStatus() == 0
    ):
