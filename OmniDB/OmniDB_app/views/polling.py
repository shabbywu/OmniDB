...
def long_polling(request):
    ...
    return JsonResponse({"returning_rows": v_returning_data})
    ...
    # Close Tab
        ...
        if v_code == requestType.Terminal:
            ...
            try:
                ...
                tab_object = create_tab_object(
                    request.session,
                    v_data["v_tab_id"],
                    {"thread": None, "terminal_object": None},
                )
                ...
                try:
                    ...
                    if v_conn_object["tunnel"]["key"].strip() != "":
                        ...
                        client.connect(
                            hostname=v_conn_object["tunnel"]["server"],
                            username=v_conn_object["tunnel"]["user"],
                            key_filename=v_full_file_name,
                            passphrase=v_conn_object["tunnel"]["password"],
                            port=int(v_conn_object["tunnel"]["port"]),
                        )
                    else:
                        client.connect(
                            hostname=v_conn_object["tunnel"]["server"],
                            username=v_conn_object["tunnel"]["user"],
                            password=v_conn_object["tunnel"]["password"],
                            port=int(v_conn_object["tunnel"]["port"]),
                        )
                    ...
                    tab_object[
                        "terminal_object"
                    ] = custom_paramiko_expect.SSHClientInteraction(
            ...
            v_code == requestType.Query
            or v_code == requestType.QueryEditData
            or v_code == requestType.SaveEditData
            or v_code == requestType.AdvancedObjectSearch
            or v_code == requestType.Console
        ):
            # create tab object if it doesn't exist
            try:
                tab_object = client_object["tab_list"][v_data["v_tab_id"]]
                ...
                tab_object = create_tab_object(
                    request.session,
                    v_data["v_tab_id"],
                    {"thread": None, "omnidatabase": None, "inserted_tab": False},
                )
        ...
        # Debugger
            ...
            if v_data["v_state"] == debugState.Starting:
                try:
                    v_conn_tab_connection = v_session.v_databases[v_data["v_db_index"]][
                        "database"
                    ]
...
def LogHistory(
    p_user_id, p_user_name, p_sql, p_start, p_end, p_duration, p_status, p_conn_id
):
    ...
    try:
        ...
        while not self.cancel:
            try:
                ...
                chunks = [
                    v_data_return[x : x + 10000]
                    for x in range(0, len(v_data_return), 10000)
                ]
        ...
        logger.error("""*** Exception ***\n{0}""".format(traceback.format_exc()))
        v_response["v_data"] = {"v_data": str(exc), "v_duration": ""}
        if not self.cancel:
            queue_response(v_client_object, v_response)
...
def thread_query(self, args):
    ...
    try:
        ...
        try:
            ...
            if (
                not v_tab_object["tab_db_id"]
                and not v_tab_object["inserted_tab"]
                and v_log_query
            ):
                try:
                    tab_object = Tab(
            ...
            if v_cmd_type == "export_csv" or v_cmd_type == "export_xlsx":
                ...
                v_file_name = "{0}.{1}".format(
                    str(time.time()).replace(".", "_"), v_extension
                )
                ...
                f = Spartacus.Utils.DataFileWriter(
                    os.path.join(v_export_dir, v_file_name),
                    v_data1.Columns,
                    v_session.v_csv_encoding,
                    v_session.v_csv_delimiter,
                )
                f.Open()
                ...
                while v_hasmorerecords:
                    v_data1 = v_database.v_connection.QueryBlock(
                        v_sql, 1000, False, True
                    )
                    if v_database.v_connection.v_start:
                        f.Write(v_data1)
                        v_hasmorerecords = False
                ...
                v_response["v_data"] = {
                    "v_filename": settings.PATH
                    + "/static/temp/{0}".format(v_file_name),
                ...
                if v_mode == 0:
                    ...
                    if (
                        not v_database.v_connection.v_con
                        or v_database.v_connection.GetConStatus() == 0
                    ):
                        v_database.v_connection.Open()
                    else:
...
def thread_console(self, args):
    ...
    try:
        ...
        try:
            ...
            v_duration = GetDuration(log_start_time, log_end_time)
            log_status = "error"
            v_response["v_data"] = {"v_data": str(exc), "v_duration": v_duration}
...
def thread_query_edit_data(self, args):
    v_response = {
        ...
        "v_context_code": args["v_context_code"],
        "v_error": False,
        "v_data": {"v_data": [], "v_row_pk": [], "v_query_info": ""},
    }
    ...
    try:
        ...
        try:
            ...
            v_data1 = v_database.QueryTableRecords(
                v_column_list, v_table_name, v_filter, v_count
            )
            ...
            for v_row in v_data1.Rows:
                ...
                for j in range(0, len(v_pk_list)):
                    ...
                    v_pk_col["v_value"] = v_row[
                        v_pk_list[j]["v_column"].replace('"', "")
                    ]
        ...
        for v_row_info in v_rows_info:
            ...
            if v_row_info["mode"] == -1:
                ...
                for v_pk in v_row_info["pk"]:
                    ...
                    # Type not found
                        ...
                        v_type_details = {"quoted": True}

                    if v_type_details["quoted"]:
                        v_command = "{0} {1} = '{2}'".format(
                            v_command, v_pk["v_column"], v_pk["v_value"]
                        )
                    else:
                        v_command = "{0} {1} = {2}".format(
                            v_command, v_pk["v_column"], v_pk["v_value"]
            ...
            # Inserting new row
                ...
                for j in range(1, len(v_data_rows[i])):
                    ...
                    # Getting details about the data type
                    try:
                        v_type_details = v_database.v_data_types[
                            v_columns[j - 1]["v_type"]
                        ]
                    # Type not found
                    except:
                        v_type_details = {"quoted": True}
