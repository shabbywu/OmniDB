...
def save_shortcuts(
    ...
    try:
        ...
        Shortcut.objects.filter(user=user).delete()
...
def get_database_list(
    ...
    json_object=Depends(dependencies.parse_json_object),
    v_session=Depends(dependencies.get_omnidb_session),
    v_cryptor=Depends(dependencies.get_cryptor),
    v_return=Depends(dependencies.get_default_return),
):
    v_databases = []
    v_groups = []
    ...
    for key, v_database_object in v_session.v_databases.items():
        if (
            v_database_object["tunnel"]["enabled"]
            or v_database_object["technology"] == "terminal"
        ):
            v_alias = ""
            if v_database_object["alias"] != "":
                v_alias = v_database_object["alias"]
            v_details = (
                v_database_object["tunnel"]["user"]
                + "@"
                + v_database_object["tunnel"]["server"]
                + ":"
                + v_database_object["tunnel"]["port"]
            )
            v_terminal_object = {
                "v_conn_id": key,
                "v_alias": v_alias,
        ...
        if v_database_object["database"] != None:
            ...
            if not v_database_object["tunnel"]["enabled"]:
                ...
                v_details = (
                    v_database.PrintDatabaseDetails()
                    + " <b>("
                    + v_database_object["tunnel"]["server"]
                    + ":"
                    + v_database_object["tunnel"]["port"]
    ...
    try:
        v_existing_tabs = []
        for tab in Tab.objects.filter(user=user).order_by("connection"):
            if tab.connection.public or tab.connection.user.id == user.id:
                v_existing_tabs.append(
                    {
...
def draw_graph(
    ...
    try:
        ...
        if v_curr_constraint != "":
            v_edge = {"from": v_curr_from, "to": v_curr_to, "label": "", "arrows": "to"}
    ...
    try:
        ...
        if v_pk != None and len(v_pk.Rows) > 0:
            ...
            for v_pk_col in v_pk_cols.Rows:
                ...
                v_first = False
                v_return["v_data"]["v_ini_orderby"] = (
                    v_return["v_data"]["v_ini_orderby"] + "t." + v_pk_col["column_name"]
                )
        v_index = 0
        for v_column in v_columns.Rows:
            v_col = {}
...
def get_completions(
    ...
    try:
        v_data1 = v_database.v_connection.GetFields(
            "select x.* from " + v_table + " x where 1 = 0"
        )
    except Exception as exc:
        v_return["v_data"] = {"password_timeout": True, "message": str(exc)}
        v_return["v_error"] = True
...
@app.post("/get_completions_table/")
def get_completions_table(
    ...
    try:
        v_data1 = v_database.v_connection.GetFields(
            "select x.* from " + v_table_name + " x where 1 = 0"
        )
    except Exception as exc:
        v_return["v_data"] = {"password_timeout": True, "message": str(exc)}
        v_return["v_error"] = True
...
def get_command_list(
    ...
    try:
        ...
        v_query = QueryHistory.objects.filter(
            user=user, connection=conn, snippet__icontains=v_command_contains
        ).order_by("-start_time")
    ...
    v_return["v_data"] = {"commandList": v_command_list, "pages": v_page}

    return v_return


@app.post("/clear_command_list/")
    ...
    try:
        ...
        v_filter = """\
            where user_id = {0}
              and conn_id = {1}
        """.format(
            str(v_session.v_user_id), str(v_database_index)
        )
...
@app.post("/get_autocomplete_results/")
            ...
            try:
                ...
                for v_type in v_data1:
                    ...
                    v_current_group["elements"].append(
                        {
                            "value": v_value + v_type.v_truename,
                            "select_value": v_value + v_type.v_truename,
        ...
        try:
            ...
            if v_search != None:
                ...
                for v_search_row in v_search.Rows:
                    ...
                    v_current_group["elements"].append(
                        {
                            "value": v_search_row["result"],
                            "select_value": v_search_row["select_value"],
                            "complement": v_search_row["complement"],
                        }
                    )
