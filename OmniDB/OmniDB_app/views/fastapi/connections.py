...
def get_connections(
    ...
    v_connection_list = []
    try:
        for conn in Connection.objects.filter(Q(user=user) | Q(public=True)):
            ...
            if conn.technology.name != "terminal":
                ...
                v_conn_object["password"] = (
                    False if conn.password.strip() == "" else True
                )
...
def get_groups(
    ...
    v_current_group_data = {"id": None, "name": None, "conn_list": []}

    try:
        for group in Group.objects.filter(user=user):
            v_current_group_data = {"id": group.id, "name": group.name, "conn_list": []}
...
def new_group(
    ...
    try:
        new_group = Group(user=user, name=p_name)
        new_group.save()
    except Exception as exc:
...
def edit_group(
    ...
    try:
        group = Group.objects.get(id=p_id, user=user)
        group.name = p_name
        group.save()
    except Exception as exc:
        v_return["v_data"] = str(exc)
        ...
        try:
            ...
            if ssh_key.strip() != "":
                ...
                with open(v_full_file_name, "w") as f:
                    f.write(ssh_key)
                client.connect(
                    hostname=json_object["tunnel"]["server"],
                    username=json_object["tunnel"]["user"],
            ...
            try:
                if ssh_key.strip() != "":
                    ...
                    server = SSHTunnelForwarder(
                        (
                            json_object["tunnel"]["server"],
                            int(json_object["tunnel"]["port"]),
                        ),
                        ssh_username=json_object["tunnel"]["user"],
                        ssh_private_key_password=ssh_password,
                        ssh_pkey=v_full_file_name,
                        remote_bind_address=(
                            database.v_active_server,
                            int(database.v_active_port),
                        ),
                        logger=None,
                    )
                else:
...
@app.post("/save_group_connections/")
def save_group_connections(
    user=Depends(dependencies.get_user),
    json_object=Depends(dependencies.parse_json_object),
    v_session=Depends(dependencies.get_omnidb_session),
    v_return=Depends(dependencies.get_default_return),
    ...
    for v_conn_data in p_conn_data_list:
        try:
            if not v_conn_data["selected"]:
                conn = GroupConnection.objects.get(
                    group=group_obj,
