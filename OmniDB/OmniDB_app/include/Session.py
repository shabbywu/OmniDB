...
class Session(object):
    ...
    def RemoveDatabase(self, p_conn_id=None):
    ...
    def DatabaseReachPasswordTimeout(self, p_database_index):
        ...
        try:
            lock_object = tunnel_locks[
                self.v_databases[p_database_index]["database"].v_conn_id
            ]
        except:
        ...
        try:
            ...
            if self.v_databases[p_database_index]["tunnel"]["enabled"]:
                ...
                if (
                    self.v_databases[p_database_index]["tunnel_object"] == None
                    or v_create_tunnel
                ):
                    try:
                        if (
                            self.v_databases[p_database_index]["tunnel"]["key"].strip()
                            != ""
                        ):
                            v_file_name = "{0}".format(str(time.time())).replace(
                                ".", "_"
                            )
                            v_full_file_name = os.path.join(
                                settings.TEMP_DIR, v_file_name
                            )
                            with open(v_full_file_name, "w") as f:
                                f.write(
                                    self.v_databases[p_database_index]["tunnel"]["key"]
                                )
                            server = SSHTunnelForwarder(
                                (
                                    self.v_databases[p_database_index]["tunnel"][
                                        "server"
                                    ],
                                    int(
                                        self.v_databases[p_database_index]["tunnel"][
                                ...
                                ssh_username=self.v_databases[p_database_index][
                                    "tunnel"
                                ]["user"],
                                ssh_private_key_password=self.v_databases[
                                    p_database_index
                                ]["tunnel"]["password"],
                                ssh_pkey=v_full_file_name,
                                remote_bind_address=(
                                    self.v_databases[p_database_index][
                                        "database"
                                    ].v_active_server,
                                    int(
                                        self.v_databases[p_database_index][
                            ...
                            server = SSHTunnelForwarder(
                                ...
                                ssh_password=self.v_databases[p_database_index][
                                    "tunnel"
                                ]["password"],
                                remote_bind_address=(
                                    self.v_databases[p_database_index][
                        ...
                        self.v_databases[p_database_index]["tunnel_object"] = str(
                            server.local_bind_port
                        )
                        self.v_databases[p_database_index][
                            "database"
                        ].v_connection.v_host = "127.0.0.1"
                        self.v_databases[p_database_index][
                            "database"
                        ].v_connection.v_port = server.local_bind_port
            ...
            if self.v_databases[p_database_index]["prompt_password"]:
                ...
                if not self.v_databases[p_database_index][
                    "prompt_timeout"
                ] or datetime.now() > self.v_databases[p_database_index][
                    "prompt_timeout"
                ] + timedelta(
                    ...
                    self.v_databases[p_database_index][
                        "database"
                    ].v_connection.v_password = ""
                    v_test = self.v_databases[p_database_index][
                        "database"
                    ].TestConnection()

                    if v_test == "Connection successful.":
                        s = SessionStore(session_key=self.v_user_key)
                        s["omnidb_session"].v_databases[p_database_index][
                            "prompt_timeout"
                        ] = datetime.now()
                        s["omnidb_session"].v_databases[p_database_index][
                            "database"
                ...
                if datetime.now() > self.v_databases[p_database_index][
                    "prompt_timeout"
    ...
    def RefreshDatabaseList(self):
        ...
        try:
            for conn in Connection.objects.filter(
                Q(user=User.objects.get(id=self.v_user_id)) | Q(public=True)
            ):
                tunnel_information = {
    ...
    def Execute(self, p_database, p_sql, p_loghistory):
        v_table = p_database.v_connection.Execute(p_sql)

        return v_table

    def Query(self, p_database, p_sql, p_loghistory):
        v_table = p_database.v_connection.Query(p_sql, True)

        return v_table

    def QueryDataLimited(self, p_database, p_sql, p_count, p_loghistory):
